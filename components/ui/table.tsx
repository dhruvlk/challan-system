...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-auto rounded-xl border border-border/50 premium-shadow bg-card"
    >
      <table
        data-slot="table"
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "p-4 align-middle [&:has([role=checkbox])]:pr-0 text-foreground font-medium",
        className
      )}
      {...props}
    />
